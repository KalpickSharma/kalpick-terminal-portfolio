...
export default function TerminalPortfolio() {
  ...
  const [historyPointer, setHistoryPointer] = useState(-1); // -1 means new line
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'ArrowUp') {
      ...
      if (historyPointer < cmdHistory.length - 1) {
        ...
        setValue(cmdHistory[newPointer]);
      }
  ...
  return (
    <div className="min-h-screen w-full bg-zinc-950 text-zinc-200 flex items-center justify-center p-4 relative overflow-hidden cursor-none">
      <CursorEffect isGreen={isGreen} />
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="binary-rain">
          {Array.from({ length: 80 }, (_, i) => (
            <div
              key={i}
              className={`binary-digit ${isGreen ? 'text-green-500 text-shadow-green' : 'text-red-500 text-shadow-red'} absolute font-mono text-lg font-bold select-none`}
              style={{
      ...
      <div
        ref={terminalRef}
        className={`${isMaximized ? 'fixed inset-0 w-full h-full rounded-none' : 'w-full max-w-5xl rounded-2xl'} flex flex-col border border-white/10 bg-gradient-to-br from-white/5 to-black/40 backdrop-blur-md ${glowClass} relative z-10 transition-all duration-300 hover:shadow-2xl ${isGreen ? 'hover:shadow-green-500/20' : 'hover:shadow-red-500/20'}`}>
        {/* Marquee Banner */}
        <div className={`relative overflow-hidden border-b ${topBorderClass} bg-gradient-to-r ${isGreen ? 'from-green-900/40 to-green-800/20' : 'from-red-900/40 to-red-800/20'} transition-all duration-300 hover:${isGreen ? 'from-green-900/50 to-green-800/30' : 'from-red-900/50 to-red-800/30'} flex-shrink-0`}>
          <div className="marquee-container py-3">
            <div className="marquee-content">
              <span className={`text-lg font-bold ${isGreen ? 'text-green-300' : 'text-red-300'} tracking-wider transition-all duration-200 hover:${isGreen ? 'text-green-200' : 'text-red-200'} hover:scale-109`}>
                Welcome to Kalpick Sharma's Space
              </span>

              <span className={`text-lg font-bold ${isGreen ? 'text-green-300' : 'text-red-300'} tracking-wider transition-all duration-200 hover:${isGreen ? 'text-green-200' : 'text-red-200'} hover:scale-105`}>
                Welcome to Kalpick Sharma's Space
              </span>
            </div>
          </div>
        ...
        {/* Window chrome */}
        <div className={`flex items-center justify-between px-4 py-3 border-b ${topBorderClass} transition-all duration-300 hover:${isGreen ? 'bg-green-900/10' : 'bg-red-900/10'} flex-shrink-0`}>
          <div className="flex items-center gap-2">
            <div className={`w-3 h-3 rounded-full ${isGreen ? 'bg-green-500/90 shadow-[0_0_15px_rgba(34,197,94,0.8)]' : 'bg-red-500/90 shadow-[0_0_15px_rgba(239,68,68,0.8)]'} transition-all duration-300 hover:scale-110 cursor-pointer`} onClick={() => window.location.reload()} title="Reload" />
            <div className={`w-3 h-3 rounded-full ${isGreen ? 'bg-green-700/70' : 'bg-red-700/70'} transition-all duration-300 hover:scale-110 cursor-pointer`} onClick={() => setIsMaximized(!isMaximized)} title="Maximize/Restore" />
        ...
        {/* Output area */}
        <div className={`p-4 sm:p-6 lg:p-8 space-y-3 overflow-y-auto custom-scrollbar ${isMaximized ? 'flex-1' : 'max-h-[65vh]'}`} aria-live="polite">
          {history.map((h, i) => (
            <div key={i}>{h}</div>
          ))}
        </div>
...
// Lusion-style Cursor Component
const CursorEffect = ({ isGreen }) => {
  ...
  return (
    <>
      <motion.div
        className="fixed top-0 left-0 w-6 h-6 z-[9999] pointer-events-none flex items-center justify-center mix-blend-difference"
